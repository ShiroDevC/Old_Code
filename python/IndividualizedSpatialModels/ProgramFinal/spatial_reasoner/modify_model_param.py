'''Module for modifiyng models. Only used for spatial Model.
All Functions in this Module are only used in the spatial model.
...
from spatial_reasoner import model_construction_param as model_builder
...
def move(item, item_coord, relation, other_coord, model):
    ...
    # basic idea: just add the item to the model closest to the other coords,
...
def shrink_dict(model):
    """
    Tests the dict dimensions and then finds the empty cols, rows and plas.
